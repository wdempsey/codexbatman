# Site Review

**Use this section to keep the documentation site clear, polished, and easy to navigate without overbuilding it.**

The goal is not a flashy redesign. The goal is a restrained, trustworthy MkDocs site that helps users understand Codex Batman quickly and move to the right next step.

## What This Section Covers

- [UI/UX Standards](ui-ux-standards.md): the lightweight rules for page purpose, hierarchy, readability, navigation, and visual polish
- repo-local review skills for page audits, navigation checks, and visual cleanup passes
- a shared standard for making small, reversible site improvements instead of one-off redesigns

## Working Principle

Site work should follow the same discipline as the rest of the repository:

- prefer minimal reversible changes
- preserve MkDocs Material-native patterns
- improve clarity before decoration
- make next actions obvious
- use structure and cross-links before introducing custom styling

## Where This Fits

- Use [UI/UX Standards](ui-ux-standards.md) before editing page structure or presentation.
- Use [Skill Library](../setup/skill-reference.md) to find the repo-local site review skills.
- Use [Examples](../examples/index.md), [Backbone Protocol](../backbone/index.md), and [Tooling Stack](../tooling/index.md) as anchor sections when reviewing cross-links and newcomer flow.
