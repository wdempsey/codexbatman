...
description: Session-start identity resolution for student sessions. Runs before any
  workflow skill. Loads the student's profile and mastery from memory/students/{name}/,
  surfaces a one-line context header, and calibrates session depth to their current
  mastery levels. If no subfolder exists, offers to register the student on the spot.
...
3. Surface a context header (see format below)
